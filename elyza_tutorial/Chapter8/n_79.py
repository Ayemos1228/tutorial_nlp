...
from elyza_tutorial.DL_NLP.tools import compute_loss
import torch
...
class MySNN(nn.Module):
	def __init__(self, feature_size=300, n_category=4):
		...
		self.linear2 = nn.Linear(512, n_category)
		# self.linear3 = nn.Linear(256, self.n_category)
	...
	def forward(self, batch_X):
		...
		"""
		lin1_X = self.linear1(batch_X) # (batch_size, 512)
		lin2_X = self.linear2(lin1_X) # (batch_size, 256)
		# lin3_X = self.linear3(lin2_X) # (batch_size, n_category)
		out = F.softmax(lin2_X, dim=-1) # (batch_size, n_category)
